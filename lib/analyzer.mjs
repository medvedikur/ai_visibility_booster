...
function evaluateAIVB012(check, page) {
  ...
  return makeVerdict(check, "NEEDS_REVIEW", "No named author/reviewer found; verify whether content is expertise-sensitive (technical guide, opinion, analysis).");
}
...
function evaluateAIVB019(check, page) {
  ...
  if (ratio >= 0.3) {
    return makeVerdict(check, "FAIL", `${missingAlt.length}/${meaningful.length} meaningful images missing alt text.`);
  }
  if (missingAlt.length > 0) {
    return makeVerdict(check, "NEEDS_REVIEW", `${missingAlt.length}/${meaningful.length} meaningful images missing alt; verify whether decorative.`);
  }
  return makeVerdict(check, "PASS", `${meaningful.length} meaningful images all have alt text.`);
}
...
function evaluateAIVB030(check, page) {
  if (page.interstitialCandidate) {
    return makeVerdict(check, "FAIL", "Cookie/consent/interstitial candidate detected in fixed-position element.");
  }
